...
DROP POLICY IF EXISTS "Users can view their own client data" ON clientes_ado_notion;
DROP POLICY IF EXISTS "Clients can view own profile" ON clientes_ado_notion;
DROP POLICY IF EXISTS "Staff can view all clients" ON clientes_ado_notion;
...
ALTER TABLE clientes_ado_notion 
...
CREATE POLICY "Clients can view own profile" ON clientes_ado_notion
  FOR SELECT USING (user_id::text = auth.uid()::text);

-- Staff: Permiso TOTAL también en la tabla principal por si acaso
CREATE POLICY "Staff can manage all clients" ON clientes_ado_notion
...
CREATE TABLE IF NOT EXISTS glucose_history (
  ...
  client_id UUID NOT NULL REFERENCES clientes_ado_notion(id) ON DELETE CASCADE,
...
CREATE POLICY "Clients can manage own glucose" ON glucose_history
  FOR ALL USING (client_id IN (SELECT id FROM clientes_ado_notion WHERE user_id::text = auth.uid()::text));
...
CREATE POLICY "Clients can view own goals" ON coach_goals
  FOR SELECT USING (client_id IN (SELECT id FROM clientes_ado_notion WHERE user_id::text = auth.uid()::text));
