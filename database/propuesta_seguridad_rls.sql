...
CREATE POLICY "Permitir alta publica" ON public.clientes_ado_notion
FOR INSERT WITH CHECK (true);

-- 2. ACCESO ALUMNOS (Lectura)
-- Regla: "El alumno solo ve su ficha si está logueado"
CREATE POLICY "Alumno ve su propia ficha" ON public.clientes_ado_notion
FOR SELECT USING (
...
CREATE POLICY "Staff control total clientes" ON public.clientes_ado_notion
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.users
    WHERE id::text = auth.uid()::text 
