...
DROP POLICY IF EXISTS "Endocrinos ven nombres de clientes" ON public.clientes_ado_notion;
CREATE POLICY "Endocrinos ven nombres de clientes" ON public.clientes_ado_notion
FOR SELECT USING (
    EXISTS (
        SELECT 1 FROM public.users
        WHERE id::text = auth.uid()::text 
