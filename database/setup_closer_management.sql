...
ALTER TABLE public.clientes_ado_notion ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'active'; -- Para estandarizar el estado

-- MIGRACIÓN DE DATOS 1: Vincular coaches por nombre
UPDATE public.clientes_ado_notion c
SET coach_id = u.id
FROM public.users u
...
UPDATE public.clientes_ado_notion
SET status = CASE 
...
SELECT 
    ...
    (SELECT COUNT(*) 
     FROM public.clientes_ado_notion c 
...
BEGIN
    ...
    IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
        ...
        IF NEW.coach_id IS NOT NULL THEN
            ...
            SET current_clients = (
                SELECT COUNT(*)
                FROM public.clientes_ado_notion
...
-- Crear trigger
DROP TRIGGER IF EXISTS trigger_update_coach_client_count ON public.clientes_ado_notion;
CREATE TRIGGER trigger_update_coach_client_count
    AFTER INSERT OR UPDATE OF coach_id, status OR DELETE
    ON public.clientes_ado_notion
    FOR EACH ROW
...
CREATE INDEX IF NOT EXISTS idx_clientes_coach_status ON public.clientes_ado_notion(coach_id, status);
