...
export const riskAlertService = {
    ...
    incrementMissedCheckins: async (clientId: string) => {
        try {
            ...
            const { data: client } = await supabase.from('clientes').select('missed_checkins_count').eq('id', clientId).single();
            const newCount = (client?.missed_checkins_count || 0) + 1;
            await supabase.from('clientes').update({ missed_checkins_count: newCount }).eq('id', clientId);
            return newCount;
        }
    },
