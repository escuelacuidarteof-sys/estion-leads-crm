...
import { Plus, Trash2, CheckCircle, XCircle, Clock, Target, Calendar, ChevronDown, ChevronUp, AlertTriangle, Trophy, ClipboardList } from 'lucide-react';
...
const CoachGoalsManager: React.FC<CoachGoalsManagerProps> = ({ clientId, isCoach }) => {
                    ...
                    {showHistory && (
                        <div className="mt-3 space-y-2 animate-in fade-in slide-in-from-top-1">
                            {closedGoals.map(goal => {
                                ...
                                return (
                                    <div key={goal.id} className={`p-3 rounded-xl border ${isAchieved ? 'bg-green-50/50 border-green-200' : 'bg-red-50/50 border-red-200'}`}>
                                        <div className="flex items-start justify-between gap-2">
                                            <div className="flex-1">
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    <span className={`flex items-center gap-1 text-xs font-bold uppercase px-2 py-0.5 rounded-full ${isAchieved ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'}`}>
                                                        {isAchieved ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                                                        {isAchieved ? 'Conseguido' : 'No Conseguido'}
                                                    </span>
                                                    <span className="text-[10px] text-slate-400">
                                                        {getGoalTypeLabel(goal.goal_type)}
                                                    </span>
                                                </div>
                                                <h4 className={`font-bold text-sm mt-1 ${isAchieved ? 'text-green-800' : 'text-red-800'}`}>{goal.title}</h4>
