...
## Errores de Produccion (Vercel/Netlify)

### "Page not found" en rutas

**Sintoma:** La app funciona en `/` pero da 404 en otras rutas.

**Causa:** El hosting no sabe que es una SPA (Single Page Application).

**Solucion para Vercel:**
Crea archivo `vercel.json` en la raiz:
```json
{
...
4. **Importante:** Redespliega despues de añadir las variables
...
```sql
SELECT property_nombre, property_correo_electr_nico, status FROM clientes_ado_notion WHERE status = 'Active';
```
...
```sql
ALTER TABLE clientes_ado_notion DISABLE ROW LEVEL SECURITY;
