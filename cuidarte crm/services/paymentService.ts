...
// Helper to get matching renewals for a month (subscription model)
async function getRenewalsForPeriod(coach: User, month: number, year: number): Promise<PaymentReference[]> {
    ...
    // Identify renewals: clients whose subscription_end falls in this month and auto_renewal is true
    clients.forEach(c => {
        if (c.status !== ClientStatus.ACTIVE && c.status !== ClientStatus.COMPLETED) return;

        // Check if client belongs to coach
        if (!isCoachMatch(coach.id, coach.name, c.coach_id)) return;

        const subEnd = c.program?.endDate;
        if (!subEnd || !c.program?.autoRenewal) return;

        const endDate = new Date(subEnd);
        if (endDate.getMonth() !== month || endDate.getFullYear() !== year) return;

        let amount = c.program?.amount || 0;

        // Calculate Commission with fee deduction
