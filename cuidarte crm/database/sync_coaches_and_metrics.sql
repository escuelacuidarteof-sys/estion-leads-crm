...
DELETE FROM public.clientes_ado_notion 
...
DROP TRIGGER IF EXISTS tr_auto_assign_coach ON public.clientes_ado_notion;
CREATE TRIGGER tr_auto_assign_coach
    BEFORE INSERT OR UPDATE ON public.clientes_ado_notion
    FOR EACH ROW
...
BEGIN
    ...
    SELECT COUNT(*) INTO v_total_active_high_ticket FROM public.clientes_ado_notion 
    WHERE property_estado_cliente = 'Activo' AND (high_ticket = TRUE OR property_tipo_de_programa::TEXT ILIKE '%premium%');

    SELECT COUNT(*) INTO v_total_paused FROM public.clientes_ado_notion WHERE property_estado_cliente = 'Pausa';

    SELECT COUNT(*) INTO v_new_signups FROM public.clientes_ado_notion
    WHERE public.safe_extract_date(property_fecha_alta) = today_date OR DATE(created_at) = today_date;

    SELECT COUNT(*) INTO v_renewals FROM public.clientes_ado_notion
    ...
    SELECT COUNT(*) INTO v_cancellations FROM public.clientes_ado_notion
