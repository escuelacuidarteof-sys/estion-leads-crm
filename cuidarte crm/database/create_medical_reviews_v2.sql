...
CREATE TABLE IF NOT EXISTS public.medical_reviews (
    ...
    -- Usamos UUID para asegurar integridad porque la tabla clientes_ado_notion usa UUID
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'clientes_ado_notion' AND column_name = 'allow_endocrine_access') THEN
        ALTER TABLE public.clientes_ado_notion ADD COLUMN allow_endocrine_access BOOLEAN DEFAULT FALSE;
    END IF;
END $$;
