...
export const DireccionDashboard: React.FC<DireccionDashboardProps> = ({ user, onNavigate }) => {
    ...
    const [statsMain, setStatsMain] = useState<CoachMetrics>({
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const stats = { main: initialProjectMetrics(), me: initialProjectMetrics() };
            ...
            allClients.forEach(c => {
                const project = (c.high_ticket ? 'me' : 'main') as 'main' | 'me';
            ...
            const totalRegistros = stats.main.registrosMes + stats.me.registrosMes;
            const totalActivosHeader = stats.main.activos + stats.me.activos;
            const totalBajasHeader = stats.main.bajas + stats.me.bajas + stats.main.abandonos + stats.me.abandonos;
            ...
            setStatsMain(finalizeStats(stats.main));
        ...
        <div className="max-w-7xl mx-auto space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm">
                    ...
                    <h3 className="text-2xl font-black text-slate-800">{statsMain.activos + statsME.activos}</h3>
                    <p className="text-xs text-slate-400 mt-1">Activos Principal ({statsMain.activos}) + ME ({statsME.activos})</p>
                </div>
                <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm">
                    <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1 flex items-center gap-2">
                ...
                <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm">
                    ...
                    {(() => {
                        ...
                        // Let's just use ADO for now if its the main one, or calculate a quick average if stats exist
                        const displayPct = statsMain.renovacionPct;
                        return (
                            <>
                                <h3 className={`text-2xl font-black ${displayPct >= 80 ? 'text-emerald-600' : 'text-amber-600'}`}>
                                    {displayPct}%
                                </h3>
                                <p className="text-xs text-slate-400 mt-1">Global Mes (Principal)</p>
                            </>
                        );
                    })()}
                </div>
                ...
                <div className="px-8 pb-8 grid grid-cols-1 lg:grid-cols-2 gap-8 mt-6">
                    ...
                    <div className="bg-slate-50/50 rounded-3xl p-6 border border-slate-200">
                        <div className="flex items-center gap-3 mb-6">
                            <div className="p-2 bg-indigo-600 rounded-lg text-white font-black text-xs">EC</div>
                            <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest">Escuela Cuid-Arte</h4>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <NumberInput label="Activos Reales" value={statsMain.activosReales} color="text-emerald-600" />
                            <NumberInput label="Fuera de Plazo" value={statsMain.fueraDePlazo} color="text-rose-500" />
                            <NumberInput label="Bajas + Abandonos" value={statsMain.bajas + statsMain.abandonos} color="text-slate-700" />
                            <NumberInput label="Churn Rate" value={`${statsMain.churnRate}%`} color={statsMain.churnRate < 5 ? 'text-emerald-600' : 'text-rose-600'} />
                        </div>
                        <div className="mt-4 p-4 bg-white rounded-2xl border border-slate-100">
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-lg font-black text-indigo-600">{statsMain.renovacionPct}%</span>
                            </div>
                        </div>
                    </div>
