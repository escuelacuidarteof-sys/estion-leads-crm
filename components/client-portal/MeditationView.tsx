...
const INITIAL_MEDITATIONS: Meditation[] = [
    ...
    },
    {
        ...
        description: 'Sesión de respiración consciente para reducir el estrés inmediato y reconectar con tu paz interior.',
        duration: '05:00',
        audioUrl: 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3',
        type: 'audio',
        category: 'Relajación',
        coverImage: 'https://images.unsplash.com/photo-1518241353349-995973907c03?q=80&w=1000&auto=format&fit=crop'
    }
];
        ...
        <div className="min-h-screen bg-[#FDFBF7] animate-in fade-in duration-500 pb-32">
            ...
            <div className="max-w-4xl mx-auto px-6 py-8">
                    ...
                    {['Todos', 'Relajación', 'Claridad', 'Sueño'].map((cat) => (
                        <button 
                            ...
                            className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all whitespace-nowrap ${
                                cat === 'Todos' ? 'bg-brand-dark text-white ring-4 ring-brand-dark/5' : 'bg-white text-slate-500 border border-slate-200 hover:border-brand-green hover:text-brand-green'
                            }`}
                        >
                            {cat}
                        </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {INITIAL_MEDITATIONS.map((meditation) => (
                        <div 
                            key={meditation.id}
                            ...
                            {currentMeditation.type === 'audio' ? (
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        modal.id = 'vocaroo-modal';
                                        modal.className = 'fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4';
                                        modal.innerHTML = `
                                            <div class="bg-white rounded-3xl p-6 w-full max-w-lg relative">
                                                <button onclick="document.getElementById('vocaroo-modal').remove()" class="absolute top-4 right-4 p-2 bg-slate-100 rounded-full"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button>
                                                <h3 class="font-heading font-black text-lg mb-4">${currentMeditation.title}</h3>
                                                <div class="rounded-2xl overflow-hidden bg-slate-50 aspect-[16/5] flex items-center justify-center">
                                                    <iframe src="${currentMeditation.audioUrl}" width="300" height="60" frameborder="0" scrolling="no"></iframe>
                                                </div>
                                                <p class="text-xs text-slate-400 mt-4 text-center">Reproducción externa vía Vocaroo</p>
                                            </div>
                                        `;
                                        document.body.appendChild(modal);
                                    }}
                                    className="bg-brand-gold text-brand-dark font-black text-xs px-4 py-3 rounded-xl hover:bg-brand-gold/90"
                                >
                                    ABRIR
                                </button>
                            )}
