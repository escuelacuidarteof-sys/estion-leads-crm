...
export function TeamAnnouncementsView({ user, clients }: TeamAnnouncementsViewProps) {
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            setAnnouncements(prev => prev.map(a =>
                a.id === editingAnnouncement.id ? { ...a, title: editTitle.trim(), message: editMessage.trim() } : a
            ));
            if (selectedAnnouncement?.id === editingAnnouncement.id) {
                setSelectedAnnouncement({ ...selectedAnnouncement, title: editTitle.trim(), message: editMessage.trim() });
            }
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
                                                ...
                                                <div className="flex items-center gap-2 shrink-0">
                                                    {(user.role === UserRole.ADMIN || user.role === UserRole.HEAD_COACH) && (
                                                        <>
                                                            ...
                                                            <button
                                                                onClick={(e) => { e.stopPropagation(); startEditing(announcement); }}
                                                                className="p-2 bg-amber-50 hover:bg-amber-100 text-amber-600 rounded-lg transition-all hover:scale-110 active:scale-95"
                                    ...
                                    {(user.role === UserRole.ADMIN || user.role === UserRole.HEAD_COACH) && (
                                        <div className="mt-6 pt-6 border-t border-slate-100 flex gap-3">
                                            <button
                                                ...
                                                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-amber-50 hover:bg-amber-100 text-amber-700 rounded-xl font-bold text-sm transition-colors"
                                            >
                                                <Pencil className="w-4 h-4" /> Editar
                                            </button>
                                            <button
                                                onClick={() => handleDelete(selectedAnnouncement.id)}
                                                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-red-50 hover:bg-red-100 text-red-700 rounded-xl font-bold text-sm transition-colors"
                                            >
            ...
            {/* Edit Modal */}
            {editingAnnouncement && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setEditingAnnouncement(null)}>
                    <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className="p-6 bg-gradient-to-r from-amber-500 to-orange-500 text-white">
                            <h3 className="text-lg font-bold flex items-center gap-2">
                                <Pencil className="w-5 h-5" /> Editar Anuncio
                            </h3>
                        </div>
                        <div className="p-6 space-y-4">
                            <div>
                                <label className="text-sm font-bold text-slate-700 mb-1 block">Título</label>
                                <input
                                    type="text"
                                    value={editTitle}
                                    onChange={e => setEditTitle(e.target.value)}
                                    className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-amber-500/20"
                                />
                            </div>
                            <div>
                                <label className="text-sm font-bold text-slate-700 mb-1 block">Mensaje</label>
                                <textarea
                                    value={editMessage}
                        ...
                        <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-end gap-3">
                            ...
                            <button
                                ...
                                className="px-6 py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-xl transition-colors disabled:opacity-50"
                            >
