...
import {
    TrendingUp, TrendingDown, DollarSign, FileText, CheckCircle,
        ...
        try {
            const { error } = await supabase
                ...
                .update({
                    ...
                    project: editingSale.project || 'ADO'
                })
        ...
        try {
            const { error } = await supabase
                ...
                .update({
                    ...
                    project: editingLeadOutcome.project || 'ADO',
                    updated_at: new Date().toISOString()
                })
    ...
    const RecordsSection = () => (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-5 text-white shadow-lg shadow-emerald-200/50 relative overflow-hidden group">
                <CheckCircle className="absolute -right-4 -bottom-4 w-24 h-24 opacity-10 group-hover:scale-110 transition-transform" />
                <div className="relative">
                    <div className="flex items-center gap-2 mb-1">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        value={editingSale.project || 'ADO'}
                                        onChange={e => setEditingSale({ ...editingSale, project: e.target.value as any })}
                                    >
                                        <option value="ADO">Escuela Cuid-Arte</option>
                        ...
                        <form onSubmit={handleUpdateLeadOutcome} className="p-6 space-y-5 overflow-y-auto">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        >
                                            <CheckCircle className="w-4 h-4 inline-block mr-1" /> SÍ
                                        </button>
                                        <button
                                            type="button"
                            ...
                            <div>
                                ...
                                <div className="flex gap-4">
                                    <button
                                        type="button"
                                        onClick={() => setEditingLeadOutcome({ ...editingLeadOutcome, project: 'ADO' })}
                                        className={`flex-1 py-3 rounded-xl border font-bold text-sm transition-all ${editingLeadOutcome.project === 'ADO' || !editingLeadOutcome.project ? 'bg-blue-600 border-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`}
                                    >
                                        Escuela Cuid-Arte
                                    </button>
                                    <button
                                        ...
                                        <div>
                                            <h3 className="font-bold text-slate-900 flex items-center gap-2">
                                                ...
                                                {sale.commission_paid && (
                                                    <CheckCircle className="w-4 h-4 text-emerald-500" title="Comision Cobrada" />
                                                )}
                                                {sale.status === 'failed' && (
                                        ...
                                        <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-slate-200">
                                            <div className="flex gap-2">
                                                {sale.status !== 'failed' ? (
                                                    ...
                                                    >
                                                        <CheckCircle className="w-3.5 h-3.5" /> Recuperar
                                                    </button>
                                                )}
                                                <button
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                >
                                                    {sale.commission_paid ? <CheckCircle className="w-3.5 h-3.5" /> : <Clock className="w-3.5 h-3.5" />}
