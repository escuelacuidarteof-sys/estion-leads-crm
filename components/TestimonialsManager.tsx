...
import {
    ...
    CheckCircle, Clock, MessageSquare, X, Save,
                    ...
                    {isRRSSorAdmin && (
                        <div className="flex items-center gap-4 text-sm font-normal">
                            <span className="flex items-center gap-1 text-emerald-600">
                                <CheckCircle size={14} /> Publicados: {testimonials.filter(t => t.is_published).length}
                            </span>
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            {t.notion_status ? (
                                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide border ${['Revision', 'Revisión'].includes(t.notion_status) ? 'bg-amber-50 text-amber-600 border-amber-200' :
                                                    ...
                                                    {['Done', 'Publicado', 'Completado'].includes(t.notion_status) && <CheckCircle size={10} />}
                                                    {t.notion_status}
                                                </span>
                                            ) : (
