    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const commonClientData = {
                ...
                status: 'active',
                client_status: 'Alta Reciente',
                registration_date: new Date().toISOString().split('T')[0],
                contracted_months: contractDuration,
                extra_notes: `NOTAS VENTA: ${formData.admin_notes || ''}\nNOTAS PARA COACH: ${formData.coach_notes || ''}`.trim(),
                onboarding_token: onboardingToken,
            ...
            if (existingClient) {
                await supabase.from('clientes').update(commonClientData).eq('id', existingClient.id);
            } else {
