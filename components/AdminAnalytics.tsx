...
const AdminAnalytics: React.FC<AdminAnalyticsProps> = ({ defaultTab = 'monthly', initialProject = 'Global' }) => {
    ...
    const [selectedProject, setSelectedProject] = useState<'Global' | 'ADO' | 'ME'>(initialProject);
    ...
    const fetchClientProfile = async () => {
        ...
        try {
            const { data, error } = await supabase
                .from('clientes')
                .select('gender, age, province, insulin_type, initial_contract_duration, status');
            ...
            data.forEach(d => {
                const dur = d.initial_contract_duration || 'Sin datos';
    ...
    const handleAutoFillFromCRM = async () => {
        ...
        try {
            ...
            const filteredClients = clients?.filter(c => {
                if (selectedProject === 'ADO') return !c.high_ticket;
